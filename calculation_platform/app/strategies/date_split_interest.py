...
class DateSplitInterestStrategy(CalculationStrategy):
    ...
    def run(self, definition, inputs: Dict[str, Any], request) -> StrategyOutcome:
        ...
        try:
            segments = sorted(
                self.parameter_store.all_effective_ranges(rate_parameter_id, start, end),
                key=lambda pv: pv.effective_from,
            )
        except KeyError as e:
            raise ParameterResolutionError(str(e), details={"parameter_id": rate_parameter_id}) from e
        if not segments:
            raise ParameterResolutionError(
                f"No rate segments found for {rate_parameter_id!r} in the requested period",
                details={"parameter_id": rate_parameter_id, "start_date": start.isoformat(), "end_date": end.isoformat()},
            )
        ...
        for pv in segments:
            ...
            resolved_params[key] = ResolvedParameter(
                name=key, value=float(rate), origin="parameter_store",
        ...
        # A rate table that stops mid-period must not silently shrink the
        # interest — the whole requested period has to be covered.
        period_days = (end - start).days + 1
        if covered_days != period_days:
            raise ParameterResolutionError(
                f"Rate parameter {rate_parameter_id!r} covers only {covered_days} of the "
                f"{period_days} days in {start.isoformat()}..{end.isoformat()}; "
                "update the rate table before computing interest for this period",
                details={
                    "parameter_id": rate_parameter_id,
    ...
    def _run_with_flat_rate(self, capital, start, end, day_count, definition, rate_param_name, rate) -> StrategyOutcome:
        ...
        resolved = ResolvedParameter(name=rate_param_name, value=float(rate), origin="caller_supplied")
        return StrategyOutcome(
            result={
                output_name: rounded_interest,
